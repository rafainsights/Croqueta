...
from croqueta.config import CLAUDE_MODELS, GROQ_MODELS, OPENAI_MODELS, GEMINI_MODELS, OLLAMA_MODELS, DEFAULT_PROVIDER, DEFAULT_MODEL, DEFAULT_ACTIONS, SKIP_PROMPT_CONFIRMATION
...
def main():
    """
    Main entry point for running all Croqueta functionalities with default settings.
    """
    # Load default settings
    provider, model = load_default_settings()
    print "Croqueta using default settings: Provider={}, Model={}".format(provider, model)
    ...
    # Use default actions
    selected_actions = DEFAULT_ACTIONS
    print "Croqueta running default actions: {}".format(", ".join(selected_actions))
    ...
    print "Croqueta callers' code {}.".format("included for additional context" if callers_code else "not included")

    # Process all actions
    for action in selected_actions:
        print "Croqueta processing action: {}".format(action)
        if not process_action(action, func, currentProgram, monitor, api_key, provider, model, callers_code):
            print "Croqueta failed to process action: {}".format(action)
            return

    print "Croqueta operations completed successfully."

class CroquetaRunAllScript(GhidraScript):

    def run(self):
        main()

if __name__ == "__main__":
    script = CroquetaRunAllScript()
    script.run()
